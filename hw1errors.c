...
char* foo(char* str, int* x) { // x is a shortened name that is not allowed (2) + function name isnt a verb (2)
    ...
    str2 = malloc(*x); // sizeof() is missing (1) + malloc return value isn't checked (1/2) 
    for (i = 0; i < *x; i++)
        str2[i] = str[*x - i]; // str[1] will never be copied (1) + must put {,} after a for loop (2)
    if (*x % 2 == 0) {
        printf("%s", str);
    }
    if (*x % 2 != 0)
